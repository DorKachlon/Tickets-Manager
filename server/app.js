...
app.get('/api/tickets', async (req, res) => {
  ...
  if (searchText) {
    const filterArr = arr.filter((obj) => obj.title.toLowerCase().includes(searchText.toLowerCase()));
    res.send(filterArr);
  } else if (searchContent) {
    const filterArr = arr.filter((obj) => obj.content.toLowerCase().includes(searchContent.toLowerCase()));
    res.send(filterArr);
  } else if (email) {
    const filterArr = arr.filter((obj) => obj.userEmail.toLowerCase().includes(email.toLowerCase()));
    res.send(filterArr);
  } else {
    res.send(arr);
  }
